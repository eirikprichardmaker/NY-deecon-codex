...
def _need(df: pd.DataFrame, col: str, where: str):
    if col not in df.columns:
        raise SchemaError(
            f"{where}: missing required column '{col}'. available={list(df.columns)[:60]}..."
        )
...
def run(ctx, log) -> int:
    """
    Output: data/processed/master_cost.parquet
    Adds: rf, erp, beta, coe, wacc
    """
    ...
    if not date_col or not price_col:
        raise SchemaError(f"prices.parquet must have date+price columns. cols={list(px.columns)[:80]}")
    ...
    if index_ticker and index_ticker in set(rets["ticker"]):
        ...
        var_m = float(mkt["mkt_ret"].var())
        for t, g in rets.groupby("ticker", sort=True):
            if t == index_ticker:
                continue
            j = g.merge(mkt, on="date", how="inner")
            if len(j) < min_obs or not np.isfinite(var_m) or var_m == 0:
                beta_map[t] = np.nan
                continue
            cov = np.cov(j["ret"].values, j["mkt_ret"].values)[0, 1]
            beta_map[t] = float(cov / var_m) if np.isfinite(cov) else np.nan
    ...
    # WACC: best-effort, else fallback
    ...
    if col_tax:
        tax = pd.to_numeric(out[col_tax], errors="coerce").fillna(tax_default).clip(0, 0.5)
    else:
        tax = pd.Series(tax_default, index=out.index)

    if col_mcap and col_debt:
        E = pd.to_numeric(out[col_mcap], errors="coerce")
        D = pd.to_numeric(out[col_debt], errors="coerce").clip(lower=0)
        V = E + D
