...
BACKUP_DIR = Path("src/_backup")
BACKUP_DIR.mkdir(parents=True, exist_ok=True)

INGEST_CODE = r'''# src/ingest_fundamentals_history.py
from __future__ import annotations
...
def _get_json(url: str, params: Dict[str, Any], timeout: int, max_retries: int) -> Any:
    ...
    - Denne wrapperen håndterer ConnectionReset/SSL/ProtocolError med backoff + jitter
    """
    backoff = 1.0
        ...
        try:
            ...
            if r.status_code >= 400:
                ...
                if r.status_code in (401, 403):
                    r.raise_for_status()
                # 429/5xx håndteres normalt av Retry, men hvis det likevel bobler opp:
...
def main() -> int:
    if TARGET.exists():
        ...
        backup = BACKUP_DIR / f"ingest_fundamentals_history.py.bak_{ts}"
        backup.write_text(TARGET.read_text(encoding="utf-8", errors="replace"), encoding="utf-8")
        print(f"OK: backup -> {backup}")
    TARGET.write_text(INGEST_CODE + "\n", encoding="utf-8")
    py_compile.compile(str(TARGET), doraise=True)
    print("OK: wrote hardened ingest_fundamentals_history.py (v2) and py_compile OK")
    return 0
